...
| `EUROFLOW_SCHEDULE_TICK_MS` | `60000`                  | How often the server fires due recurring requests |
...
Set up a recurring payment request — rent collection, recurring shared bills,
subscriptions. Each time a schedule is due it generates a normal payment request
to the payer (who settles it via the request flow) and advances. A background
loop fires due schedules every `EUROFLOW_SCHEDULE_TICK_MS` (default 60s); admins
can also trigger processing on demand.

| Method & path                     | Auth  | Notes                                                              |
| --------------------------------- | ----- | ------------------------------------------------------------------ |
| `POST /schedules`                 | yes   | `amount`, `frequency` (`once`/`weekly`/`monthly`), payer handle; optional `reason`, `start_date`, `end_date` |
| `GET /schedules?box=outgoing`     | yes   | Your schedules; `outgoing` (you collect) or `incoming` (you pay)   |
| `GET /schedules/:id`              | yes   | Owner or payer can view                                            |
| `POST /schedules/:id/:action`     | yes   | Owner only; `:action` ∈ `pause` \| `resume` \| `cancel`            |
| `POST /admin/schedules/run-due`   | admin | Process all due schedules now (returns `{ ran, due_schedules }`)   |

Monthly advances preserve the day-of-month (clamped to the month's length); an
overdue schedule catches up one request per missed period.

### Contacts & discovery

Find who in your address book is already on EuroFlow, and search the user
directory to find someone to pay. The directory never exposes other users'
email or phone — only name and username.

| Method & path              | Auth | Notes                                                                 |
| -------------------------- | ---- | --------------------------------------------------------------------- |
| `POST /contacts/sync`      | yes  | Body `{ contacts: [{ name?, phone?, email? }] }`; upserts and flags which are on-platform. Returns `{ added, updated, skipped, total, on_platform }` |
| `GET /contacts`            | yes  | Your contacts, each with `on_platform` + matched handle; `?on_platform=true` filters. Membership is resolved live, so a contact who joins later shows up automatically |
| `DELETE /contacts/:id`     | yes  | Remove a contact                                                      |
| `GET /users/search?q=`     | yes  | Directory: exact email/phone or username/name prefix (min 2 chars), excludes you |
| `POST /contacts/invite`    | yes  | Referral message + link for a non-member contact                     |

### Notifications

In-app notifications are generated automatically as side effects of other
actions (money received/sent, payment requests and their settlement, split
invitations and share payments, and admin merchant/KYC decisions).

| Method & path                     | Auth | Notes                                              |
| --------------------------------- | ---- | -------------------------------------------------- |
| `GET /notifications`              | yes  | Newest first; `?unread=true` filters. Response includes `unread_count` |
| `POST /notifications/:id/read`    | yes  | Mark one read (404 if it isn't yours)              |
| `POST /notifications/read-all`    | yes  | Mark all read; returns `{ marked }`                |
...
src/
  ...
  services/
    ...
    notifications.js   # in-app notification feed (generated by other actions)
    scheduled.js       # recurring payment-request schedules + due-processing
    contacts.js        # address-book sync, membership matching, user directory
public/
